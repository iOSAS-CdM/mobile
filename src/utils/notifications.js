...
export async function registerPushToken(token, accessToken) {
	try {
		...
		const response = await fetch(`${apiUrl}/push-tokens/register`, {
			method: 'POST',
			headers: {
 ...
 */
export async function setupPushNotifications(accessToken) {
	try {
		// Request permissions
 ...
 */
export async function unregisterPushToken(accessToken) {
	try {
		const apiUrl = process.env.EXPO_PUBLIC_API_URL || 'http://localhost:3001';

		const response = await fetch(`${apiUrl}/push-tokens/unregister`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
				'Authorization': `Bearer ${accessToken}`,
